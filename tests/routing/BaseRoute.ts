...
import BaseRoute = require('../../routing/BaseRoute');
...
registerSuite({
	...
	'#isCaseSensitive': function () {
		...
		assert.throws(
			function () { route.serialize({ foo: 'FOO', bar: 1234, baz: 'HI' }); },
			Error,
			null,
			'attempting to serialize a path with incorrect case should throw in case-sensitive mode'
		);

		route.set('isCaseSensitive', false);

		assert.isTrue(route.test('FOO/1234/HI'),
			'route with different case should test to true in case-insensitive mode');
	...
	'#serialize': function () {
		assert.strictEqual(
			route.serialize({ foo: 'foo', bar: 1234, baz: 'hi' }),
			'foo/1234/hi',
			'serialized route should match path definition'
		);
		assert.strictEqual(
			route.serialize({ foo: 'foo', bar: 1234, baz: 'hi', blah: 'biz', biz: 'blah' }),
			'foo/1234/hi?blah=biz&biz=blah',
		...
		assert.throws(
			...
			Error,
			null,
			'attempting to serialize without all required arguments should throw'
		);

		assert.throws(
			function () { route.serialize({ foo: 'bar', bar: 1234, baz: 'hi' }); },
			Error,
			null,
			'attempting to serialize a path with non-matching arguments should throw'
		);
	},
